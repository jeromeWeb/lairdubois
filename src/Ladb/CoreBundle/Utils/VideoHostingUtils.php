...
class VideoHostingUtils extends AbstractContainerAwareUtils
{
    ...
    public function getKindAndEmbedIdentifier($url)
    {
        ...
        if (!is_null($url)) {
            ...
            elseif (preg_match('~(?:http|https|)(?::\/\/|)(?:www.|)(?:youtu\.be\/|youtube\.com(?:\/embed\/|\/v\/|\/watch\?v=|\/ytscreeningroom\?v=|\/feeds\/api\/videos\/))([\w\-]{11})[a-z0-9;:@#?&%=+\/\$_.-]*~i', $url, $match)) {
                $kind = VideoHostingUtils::KIND_YOUTUBE;
                $embedIdentifier = $match[1];
            } // Dailymotion
            elseif (preg_match('%(?:dailymotion\.com/video|dai\.ly)/([a-zA-Z0-9]{7}|[A-Za-z0-9]{6})%i', $url, $match)) {
                $kind = VideoHostingUtils::KIND_DAILYMOTION;
                $embedIdentifier = $match[1];
            } // Vimeo
            elseif (preg_match('%vimeo\.com\/([0-9]{1,10})%i', $url, $match)) {
                $kind = VideoHostingUtils::KIND_VIMEO;
                $embedIdentifier = $match[1];
            } // Facebook
            elseif (preg_match('%^http(?:s?):\/\/www\.facebook\.com\/(?:video.php\?v=|[a-zA-Z0-9.]+\/videos\/)(\d+)%i', $url, $match)) {
